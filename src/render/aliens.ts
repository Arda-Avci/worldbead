...
function buildShip(): Omit<ShipSlot, 'active' | 'id' | 'phase' | 'from' | 'to' | 'approachT' | 'approachDur' | 'hoverSeed' | 'chargeProgress' | 'laserT' | 'laserDur' | 'laserTarget' | 'explodeT' | 'explodeDur'> {
  ...
  // empirically (against `scratchpad/invasion2` screenshots, gameplay camera
  // ~5.6 world units out) so a hovering ship reads at roughly 1/6 of the
  // screen width including its fins.
  ...
  // hull's own rim.
  const finMat = new THREE.MeshStandardMaterial({ color: 0x525f70, metalness: 0.7, roughness: 0.45 });
...
export class AlienInvasionRenderer {
  ...
  /** Ship ids currently hittable (approaching/hover/charging, not mid-explosion), nearest first. */
  raycastShips(raycaster: THREE.Raycaster): number[] {
    const hittable = this.slots.filter((s) => s.active && s.phase !== 'exploding');
    const hulls = hittable.map((s) => s.hull);
    const hits = raycaster.intersectObjects(hulls, false);
    const out: number[] = [];
    for (const h of hits) {
      const slot = hittable.find((s) => s.hull === h.object);
      if (slot && !out.includes(slot.id)) out.push(slot.id);
    }
    return out;
  }
